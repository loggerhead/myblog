...
(function (root, factory) {
  ...
  function Bag(options) {
    ...
    this.stores = _isArray(options.stores) ? options.stores : ['localstorage', 'indexeddb', 'websql'];
    ...
    var cssRelativeUrlRe = /url\(('.*?'|".*?"|.*?)\)/mg;

    function patchRelativeUrlInCss(obj) {
      var refUrl = parse_url(obj.url);
      var done = false;
      var res = obj.data.replace(cssRelativeUrlRe, function (match, p1) {
        if (!match) return null;
        done = true;
        // select matched group of params
        var mapUrl = parse_url(p1);
    ...
    var handlers = {
      ...
      'text/css': function injectStyle(obj) {
        ...
        // try to change sourcemap address to absolute
        txt = patchMappingUrl(obj);
        obj.data = txt ? txt : obj.data + '\n/*# sourceURL=' + obj.url + ' */';
        txt = patchRelativeUrlInCss(obj);
        if (!txt) {
          txt = obj.data;
        }
